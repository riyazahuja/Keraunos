...
def capture_app_window(drone_id, app_title = 'DE FPV'):
    ...
    with mss.mss() as sct:
        ...
        target_region = get_target_region(width, height, (0.5,0.5), (1.5,1.5))
        ...
        while True:
            ...
            for pic, contour in enumerate(contours): 
                ...
                if(area > 450) and (maxArea is None or area > maxArea):
                    maxArea = area
                    maxContour = contour
            ...
            if(maxContour is not None):
                ...
                print(data_out)
                time_data = datetime.fromisoformat(data_out['time'])
                td = datetime.now() - time_data
    ...
    try:
        ...
        with open(path,'w') as file:
            if 'drones' not in fc.keys():
                fc['drones']={}
            if 'drones' not in fc[drone_id].keys():
                fc['drones'][drone_id]={}
            for k,v in data.items():
                fc['drones'][drone_id][k]=v
